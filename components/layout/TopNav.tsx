  ...
  return (
    <header className="sticky top-0 z-30 hidden border-b border-border/70 bg-card/85 backdrop-blur md:block">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
        <Link href={session ? "/dashboard" : "/"} className="font-display text-xl font-bold text-primary">
          SA Sourcing Hub
        </Link>
        <nav className="flex items-center gap-5 text-sm font-semibold text-muted-foreground">
          {session ? (
            <>
              ...
              {session.user.role === "admin" ? <Link href="/admin" className="hover:text-primary">Admin</Link> : null}
              <NotificationBell />
